-- Please see lang_enus.lua for reference.

QuestHelper_Translations["afZA"] =
 {
  -- Displayed by locale chooser.
  LOCALE_NAME = "PORN",
  
  -- Messages used when starting.
  LOCALE_ERROR = "PORN",
  ZONE_LAYOUT_ERROR = nil,
  DOWNGRADE_ERROR = "PORN",
  HOME_NOT_KNOWN = "PORN",
  
  -- Route related text.
  ROUTES_CHANGED = nil,
  HOME_CHANGED = "PORN",
  TALK_TO_FLIGHT_MASTER = nil,
  TALK_TO_FLIGHT_MASTER_COMPLETE = nil,
  WILL_RESET_PATH = nil,
  UPDATING_ROUTE = nil,
  
  -- Locale switcher.
  LOCALE_LIST_BEGIN = "PORN",
  LOCALE_CHANGED = "PORN",
  LOCALE_UNKNOWN = "PORN",
  
  -- Words used for objectives.
  SLAY_VERB = nil,
  ACQUIRE_VERB = "PORN",
  
  OBJECTIVE_REASON = nil, -- %1 is a verb, %2 is a noun (item or monster)
  OBJECTIVE_REASON_FALLBACK = nil,
  OBJECTIVE_REASON_TURNIN = nil,
  OBJECTIVE_PURCHASE = nil,
  OBJECTIVE_TALK = nil,
  OBJECTIVE_SLAY = nil,
  OBJECTIVE_LOOT = nil,
  
  ZONE_BORDER = nil,
  
  -- Stuff used in objective menus.
  PRIORITY = nil,
  PRIORITY1 = nil,
  PRIORITY2 = nil,
  PRIORITY3 = nil,
  PRIORITY4 = nil,
  PRIORITY5 = nil,
  SHARING = nil,
  SHARING_ENABLE = nil,
  SHARING_DISABLE = nil,
  IGNORE = "PORN",
  
  IGNORED_PRIORITY_TITLE = "PORN",
  IGNORED_PRIORITY_FIX = "PORN",
  IGNORED_PRIORITY_IGNORE = "PORN",
  
  -- Custom objectives.
  RESULTS_TITLE = nil,
  NO_RESULTS = nil,
  CREATED_OBJ = "PORN",
  REMOVED_OBJ = nil,
  USER_OBJ = nil,
  UNKNOWN_OBJ = nil,
  
  SEARCHING_STATE = nil,
  SEARCHING_LOCAL = nil,
  SEARCHING_STATIC = nil,
  SEARCHING_ITEMS = nil,
  SEARCHING_NPCS = nil,
  SEARCHING_ZONES = nil,
  SEARCHING_DONE = nil,
  
  -- Shared objectives.
  PEER_TURNIN = nil,
  PEER_LOCATION = nil,
  PEER_ITEM = nil,
  PEER_OTHER = nil,
  
  PEER_NEWER = nil,
  PEER_OLDER = nil,
  
  UNKNOWN_MESSAGE = nil,
  
  -- Hidden objectives.
  HIDDEN_TITLE = "PORN",
  HIDDEN_NONE = "PORN",
  DEPENDS_ON_SINGLE = "PORN",
  DEPENDS_ON_COUNT = "PORN",
  FILTERED_LEVEL = "PORN",
  FILTERED_ZONE = "PORN",
  FILTERED_COMPLETE = "PORN",
  FILTERED_BLOCKED = "PORN",
  FILTERED_USER = "PORN",
  FILTERED_UNKNOWN = "PORN",
  
  HIDDEN_SHOW = "PORN",
  DISABLE_FILTER = "PORN",
  FILTER_DONE = "PORN",
  FILTER_ZONE = "PORN",
  FILTER_LEVEL = "PORN",
  FILTER_BLOCKED = "PORN",
  
  -- Nagging. (This is incomplete, only translating strings for the non-verbose version of the nag command that appears at startup.)
  NAG_MULTIPLE_NEW = nil,
  NAG_SINGLE_NEW = nil,
  NAG_ADDITIONAL = nil,
  
  NAG_NOT_NEW = nil,
  NAG_NEW = nil,
  NAG_INSTRUCTIONS = nil,
  
  NAG_SINGLE_FP = nil,
  NAG_SINGLE_QUEST = nil,
  NAG_SINGLE_ROUTE = nil,
  NAG_SINGLE_ITEM_OBJ = nil,
  NAG_SINGLE_OBJECT_OBJ = nil,
  NAG_SINGLE_MONSTER_OBJ = nil,
  NAG_SINGLE_EVENT_OBJ = nil,
  NAG_SINGLE_REPUTATION_OBJ = nil,
  
  NAG_MULTIPLE_FP = nil,
  NAG_MULTIPLE_QUEST = nil,
  NAG_MULTIPLE_ROUTE = nil,
  NAG_MULTIPLE_ITEM_OBJ = nil,
  NAG_MULTIPLE_OBJECT_OBJ = nil,
  NAG_MULTIPLE_MONSTER_OBJ = nil,
  NAG_MULTIPLE_EVENT_OBJ = nil,
  NAG_MULTIPLE_REPUTATION_OBJ = nil,
  
  -- Stuff used by dodads.
  PEER_PROGRESS = nil,
  TRAVEL_ESTIMATE = nil,
  TRAVEL_ESTIMATE_VALUE = nil,
  WAYPOINT_REASON = nil,

  -- QuestHelper Map Button
  QH_BUTTON_TEXT = nil,
  QH_BUTTON_TOOLTIP1 = nil,
  QH_BUTTON_TOOLTIP2 = nil,
  QH_BUTTON_SHOW = nil,
  QH_BUTTON_HIDE = nil,

  MENU_CLOSE = "vPORN",
  MENU_SETTINGS = nil,
  MENU_ENABLE = "PORN",
  MENU_DISABLE = "PORN",
  MENU_OBJECTIVE_TIPS = nil,
  MENU_TRACKER_OPTIONS = nil,
  MENU_QUEST_TRACKER = nil,
  MENU_TRACKER_LEVEL = nil,
  MENU_TRACKER_QCOLOUR = nil,
  MENU_TRACKER_OCOLOUR = nil,
  MENU_TRACKER_SCALE = nil,
  MENU_TRACKER_RESET = nil,
  MENU_FLIGHT_TIMER = "PORN",
  MENU_ANT_TRAILS = "PORN",
  MENU_WAYPOINT_ARROW = nil,
  MENU_MAP_BUTTON = "PORN",
  MENU_ZONE_FILTER = nil,
  MENU_DONE_FILTER = "PORN",
  MENU_BLOCKED_FILTER = "PORN",
  MENU_LEVEL_FILTER = "PORN",
  MENU_LEVEL_OFFSET = "PORN",
  MENU_ICON_SCALE = "vPORN",
  MENU_FILTERS = "PORN",
  MENU_PERFORMANCE = nil,
  MENU_LOCALE = "PORN",
  MENU_PARTY = nil,
  MENU_PARTY_SHARE = nil,
  MENU_PARTY_SOLO = nil,
  MENU_HELP = "PORN",
  MENU_HELP_SLASH = "PORN",
  MENU_HELP_CHANGES = "PORN",
  MENU_HELP_SUBMIT = "PORN",
  
  -- Added to tooltips of items/npcs that are watched by QuestHelper but don't have any progress information.
  -- Otherwise, the PEER_PROGRESS text is added to the tooltip instead.
  TOOLTIP_WATCHED = nil,
  TOOLTIP_QUEST = nil,
  TOOLTIP_PURCHASE = nil,
  TOOLTIP_SLAY = nil,
  TOOLTIP_LOOT = nil
 }


...
QuestHelper_Translations.ruRU =
 {
  ...
  PLEASE_DONATE = "%h(Дело КвестХелпера живет благодаря Вашим пожертвованиям!) Мы будем благодарны за Все, что вы сможете пожертвовать - даже несколько долларов в месяц позволит быть уверенным, что я продолжу обновлять и работать над этим аддоном. Введите %h(/qh donate) для информации.",
  ...
  NAG_MULTIPLE_NEW = "У вас есть %h(новая информация) по %h1 и %h(обновленная информация) по %h2.",
  NAG_SINGLE_NEW = "У вас есть %h(новая информация) по %h1.",
  NAG_ADDITIONAL = "У вас есть %h(дополнительная информация) по %h1.",
